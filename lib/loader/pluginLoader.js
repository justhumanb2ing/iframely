    ...
    function shouldLoadDomainPlugin(bits) {
        if (!CONFIG.ENABLED_DOMAIN_PLUGINS) {
            return true;
        }
        ...
        var domain = getFileName(bits[domainBitIdx + 1]).replace(/^www\./i, "");
        return CONFIG.ENABLED_DOMAIN_PLUGINS.indexOf(domain) > -1;
    }
    ...
    async function loadPluginFile(pluginPath) {
        ...
        if (pluginPath.match(/\.js$/i)) {
            ...
            if (!shouldLoadDomainPlugin(bits)) {
                return;
            }
